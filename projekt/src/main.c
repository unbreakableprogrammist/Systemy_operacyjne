...
void remove_newline(char *line) {
  line[strcspn(line, "\n")] = 0;
} // strcspn znajduje w line pierwszy znak z \n i zwraca jego indeks
...
void delete_node(struct wezel **head_ref, char *source_path,
                 char *destination_path) {
  struct wezel *tmp = *head_ref;
  struct wezel *prev = NULL;
  while (tmp != NULL) {
    // jesli source path i destination path są takie same jak te z wezla
    if (strcmp(tmp->source_path, source_path) == 0 &&
        strcmp(tmp->destination_path, destination_path) == 0) {
...
int tokenize(char *line, char *args[]) {
  ...
  while (*src && arg_count < MAXARGS) { // dopoki src jest rozny od \0 i
                                        // arg_count jest mniejszy od MAXARGS
    while (*src == ' ' &&
           !in_quote) // omijamy nadmairowe spacje i nie wewnątrz cudzysłowia
      src++;
    ...
    while (*src) {           // dopoki src jest rozny od \0
      if (*src == '"') {
        ...
        src++;
      } else if (*src == ' ' && !in_quote) { // jesli src jest spacja i nie
...
int is_subpath(const char *parent, const char *child) {
  ...
  if (realpath(parent, real_parent) ==
      NULL) { // zrodlo musi na pewno istniec , inaczej blad programu
    perror("realpath");
    return EXIT_FAILURE;
  }
  ...
  if (realpath(child, real_child) == NULL) { // jesli cel nie istnieje
    // Cel nie istnieje. Musimy rozwiązać jego katalog nadrzędny ("parent
    // directory" celu). Np. dla child="../dst1", parent_part="..",
    // name_part="dst1" przepisujemy do temp child taka sciezke jaka dostalismy
    ...
    if (last_slash == NULL) {
      ...
      if (last_slash ==
          temp_child) { // Przypadek "/dst1" , czyli przypadek z rootem
        strcpy(parent_part, "/");
      } else {
    ...
    if (realpath(parent_part, real_parent_part) ==
        NULL) { // oznacza folder nie istnieje -> mozemy go stworzyc
      return 0;
    }
...
// Funkcja sprawdza, czy para <zrodlo> <cel> już istnieje w liście aktywnych
// procesów. Zwraca 1 (DUPLIKAT), 0 (OK)
int is_duplicate(struct wezel *head, const char *src, const char *dst) {
  ...
  while (tmp != NULL) {
    ...
    if (strcmp(tmp->source_path, src) == 0 &&
        strcmp(tmp->destination_path, dst) == 0) {
      return 1;
    }
    tmp = tmp->next;
  }
...
int main() {
  ...
  printf("Start programu. Komendy: add <src> <dst1>..., list, end <src> <dst>, "
         "exit, help\n");
  // ignorujemy sygnały inne niż SIGINT, SIGTERM, SIGCHLD, SIGKILL, SIGSTOP,
  // SIGSEGV, SIGFPE (SIGSEGV i SIGFPE sygnaly do handlowania bledow segfoult i
  ...
  for (int i = 1; i < NSIG; i++) {
    if (i != SIGINT && i != SIGTERM && i != SIGCHLD && i != SIGKILL &&
        i != SIGSTOP && i != SIGSEGV && i != SIGFPE) {
  ...
  while (running) {
    if (fgets(line, sizeof(line), stdin) ==
        NULL) { // bierz cala linijke wejsciowa az d- '\n'( null oznacza blad)
      if (errno == EINTR)
    ...
    if (strcmp(args[0], "exit") ==
        0) { // jesli pierwszy argument to exit to zamykamy petle
      running = 0;
      break;
    }
    ...
    if (strcmp(args[0], "add") == 0) {
      ...
      for (int i = 2; i < argc; i++) {
        ...
        if (pid ==
            0) { // jesli jest dzieckiem to uruchom file_watcher_reccursive
        ...
        nowy->destination_path[MAX_PATH - 1] =
            0;             // ustawiamy ostatni znak na \0
