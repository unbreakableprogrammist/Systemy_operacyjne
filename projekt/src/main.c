...
{  // przekazujemy wskaźnik na początek listy
...
int is_subpath(const char *parent, const char *child)
{
    ...
    {  // zrodlo musi na pewno istniec , inaczej blad programu
        perror("realpath");
        return EXIT_FAILURE;
    }
    ...
    {  // jesli cel nie istnieje
        ...
        else
        {
            ...
            if (last_slash == temp_child)
            {  // Przypadek "/dst1" , czyli przypadek z rootem
                strcpy(parent_part, "/");
            }
            else
            {
        ...
        {  // oznacza folder nie istnieje -> mozemy go stworzyc
            return 0;
        }
...
int main()
{
    ...
    printf(
        "Start programu. Komendy: add <src> <dst1>..., list, end <src> <dst>, "
        "exit, help\n");
    ...
    while (running)
    {
        ...
        {  // bierz cala linijke wejsciowa az d- '\n'( null oznacza blad)
            if (errno == EINTR)
        ...
        {  // jesli pierwszy argument to exit to zamykamy petle
            running = 0;
            break;
        }
        ...
        if (strcmp(args[0], "add") == 0)
        {
            ...
            for (int i = 2; i < argc; i++)
            {
                ...
                if (pid == -1)
                {  // jesli blad wypisz blad i kontynuuj
                    perror("fork");
                    continue;
                }

                if (pid == 0)
                {  // jesli jest dzieckiem to uruchom file_watcher_reccursive
