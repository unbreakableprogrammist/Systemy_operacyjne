...
void handle_link(char *source, char *destination, char *root_src,
  ...
  if (new_target[0] == '/' &&
      strncmp(link_target, root_src, strlen(root_src)) ==
          0) { // jesli zaczynamy sie od / i nasza sciezka na ktora wskazujemy
               // jest podobna do folderu skad przyszlismy to musimy zmienic
    char *suffix =
        link_target +
        strlen(
            root_src); // obliczamy suffix sciezki ( to co sie powtarza po src)
...
void directory_copy(char *source_path, char *destination_path) {
  ...
  if (stat(destination_path, &st_dest) ==
      0) { //  pobieramy informacje o destination_path i zapisujemy do st_dest ,
           //  0 = succes
    ...
    if (S_ISDIR(st_dest.st_mode)) {
      ...
      if (!is_dir_empty(destination_path)) {
        fprintf(stderr,
                "[PID %d] BŁĄD: Katalog docelowy '%s' nie jest pusty!\n",
      ...
      fprintf(stderr,
              "[PID %d] BŁĄD: '%s' istnieje, ale nie jest katalogiem.\n",
              getpid(), destination_path);
      exit(EXIT_FAILURE);
    }
  } else {
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    int n1 = snprintf(
        next_src, PATH_MAX, "%s/%s", source_path,
        ...
        if (w != NULL) {
          ...
          if (event->mask & IN_ISDIR) { // czy event dotyczy katalogu
            if (event->mask & IN_CREATE ||
                event->mask & IN_MOVED_TO) { // jesli dotyczy tworzenia katalogu
              ...
              add_watches_recursive(notify_fd, full_src, full_dst);
                       ...
                       event->mask &
                           IN_MOVED_FROM) { // jesli dotyczy usuwania katalogu
