...
// funkcja sprawdzajaca czy katalog jest pusty , 1 - pusty , 0 - niepusty , -1 -
// blad
int is_dir_empty(const char *path)
{
    DIR *dir = opendir(path);
    if (!dir)
        return -1;  // Pewnie nie istnieje, więc jest "czysty" lub blad opendir
    struct dirent *entry;
...
void add_watch_to_map(int wd, const char *src, const char *dst)
{
    ...
    {                                            // sprawdzamy czy nie przekroczylismy
                                                 // maksymalnej liczby watchow
        map.watch_map[map.watch_count].wd = wd;  // dodajemy wd
        strncpy(map.watch_map[map.watch_count].src, src,
                PATH_MAX - 1);  // kopiujemy sciezke src
        strncpy(map.watch_map[map.watch_count].dst, dst,
                PATH_MAX - 1);  // kopiujemy sciezke dst
...
{
    int src_fd = open(source,
                      O_RDONLY);  // otwieramy plik zrodlowy ale w trybie tylko do odczytu
    if (src_fd < 0)
    {
        perror("open source failed");  // nie wywolujemy ERR bo nieodczytanie jednego
                                       // pliku nie powoduje zatrzymania
        return;
    }
    // otwieramy plik docelowy ale w trybie tylko do zapisu , O_TRUNC zeruje plik
    // jesli istnieje , 644 - standardowe uprawnienia
...
void handle_link(char *source, char *destination, char *root_src, char *root_dst)
{
    ...
    snprintf(new_target, sizeof(new_target), "%s",
             link_target);  // na razie po prostu przepisujemy
    if (new_target[0] == '/' && strncmp(link_target, root_src, strlen(root_src)) == 0)
    {                                                   // jesli zaczynamy sie od / i nasza sciezka na ktora wskazujemy
                                                        // jest podobna do folderu skad przyszlismy to musimy zmienic
...
void directory_copy(char *source_path, char *destination_path)
{
    ...
    {  //  pobieramy informacje o destination_path i zapisujemy do st_dest ,
       //  0 = succes
    ...
    else
    {
        ...
        // katalogu
        if (mkdir(destination_path, st_src.st_mode & 0777) < 0 && errno != EEXIST)
        {  // EEXIST = katalog juz istnieje
    ...
    if (!dir)
    {
        // Jeśli nie możemy otworzyć źródła, to backup tego fragmentu jest
        // niemożliwy
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        int n1 = snprintf(next_src, PATH_MAX, "%s/%s", source_path,
                          entry->d_name);  // zklejamy poprzednia z nazwa obecnego pliku/katalogu
        int n2 = snprintf(next_dst, PATH_MAX, "%s/%s", destination_path,
                          entry->d_name);  // tak samo sklejamy z docelowym
        ...
        if (lstat(next_src, &st) == 0)
        {
            if (S_ISDIR(st.st_mode))
            {
                ...
                // a potem wchodzimy do środka (katalog jest pusty, więc walidacja na
                // początku przejdzie)
...
{
    int wd =
        inotify_add_watch(notify_fd, source_path,
                          IN_CREATE | IN_DELETE | IN_MODIFY | IN_MOVED_TO | IN_MOVED_FROM | IN_DELETE_SELF | IN_ATTRIB);
...
void file_watcher_reccursive(char *source_path, char *destination_path)
{
    ...
    while (running)
    {
        ...
        while (i < len)
        {
            struct inotify_event *event = (struct inotify_event *)&buffer[i];  // bierzemy jedno wydarzenie z
